// src/components/report/InterpretationTabV32.jsx
// VS-32: Simplified AI interpretation - single call, no questions
// Uses new v32 endpoints with evidence tag stripping

import React, { useState, useEffect, useCallback, useRef } from 'react';
import { supabase } from '../../lib/supabase';
import { processInterpretationSections } from '../../utils/evidence';
import { Sparkles, AlertCircle, RefreshCw, Clock, CheckCircle } from 'lucide-react';
...
// Section icons based on ID
const SECTION_ICONS = {
  executive_snapshot: Sparkles,
  strengths: CheckCircle,
  constraints: AlertCircle,
  opportunity_areas: RefreshCw,
  path_forward: Clock,
};

export default function InterpretationTabV32({ runId }) {
    ...
    const poll = async () => {
      ...
      try {
        ...
        if (data.status === 'completed') {
          // Process sections to strip evidence tags
      ...
      <div className="bg-white border border-slate-200 rounded-sm p-6">
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1">
            <h3 className="text-base font-semibold text-slate-900">AI-Powered Insights</h3>
            <p className="text-sm text-slate-500 mt-1 mb-4">
              Generate personalized analysis and strategic recommendations based on your assessment.
              Our AI synthesizes your data into five actionable sections.
            </p>
            <button
    ...
    return (
      <div className="space-y-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500">
                  {usedFallback ? 'Generated with template assistance' : 'Generated from your assessment data'}
                </p>
              </div>
            </div>
            {canRegenerate && (
        ...
        {sections.map((section, index) => {
          const IconComponent = SECTION_ICONS[section.id] || Sparkles;
          return (
            <div key={section.id || index} className="bg-white border border-slate-200 rounded-sm p-5">
