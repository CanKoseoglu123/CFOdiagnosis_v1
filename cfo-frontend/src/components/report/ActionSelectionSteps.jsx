    ...
    <div className="flex flex-col h-full">
            ...
            {SUB_STEPS.map((step) => {
              ...
              return (
                <button
                  ...
                  title={step.label}
                  className={`w-8 h-8 flex items-center justify-center text-sm font-semibold rounded-sm border transition-colors ${
                    isActive
      ...
      <div className="px-6 py-3 border-t border-slate-200 bg-slate-50 flex items-center justify-between flex-shrink-0">
        ...
        <div className="text-sm text-slate-600">
          <span className="font-semibold">{selectedActions.size}</span> action{selectedActions.size !== 1 ? 's' : ''} selected total
        </div>

        <button
          onClick={handleContinue}
          className="flex items-center gap-2 px-3 py-1.5 text-sm font-medium rounded-sm transition-colors bg-slate-700 text-white hover:bg-slate-800"
        >
          {isLastSubStep ? 'Continue to Timelines' : 'Continue'}
